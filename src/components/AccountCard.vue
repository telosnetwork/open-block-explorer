...
export default defineComponent({
    ...
    setup(props) {
        ...
        const determineRamUnit = (size: number) => {
        ...
        const loadResources = () => {
            ...
            if (props.account !== system_account.value) {
                // in most cases the max value unit is more im portant than the
                // used value unit. So we use the max value unit.
                // get max ram number
                const ramMaxNumber = Number(accountData.value.ram_quota);
                // get denominator based on max number
                ramDenominator = determineRamUnit(ramMaxNumber).denominator;
                // get displayed number value
                ram_max.value = fixDec(
                    ramMaxNumber / ramDenominator,
                );
                // get units
                ramUnit.value = determineRamUnit(ramMaxNumber).unit;
                const netUsedNumber = Number(accountData.value.net_limit.used);
                const netMaxNumber = Number(accountData.value.net_limit.max);
                const netDenominator = determineRamUnit(netMaxNumber).denominator;
                netUnit.value = determineRamUnit(netMaxNumber).unit;
            ...
            if (!ramDenominator) {
                ramDenominator = determineRamUnit(ramUsedNumber).denominator;
                ramUnit.value = determineRamUnit(ramUsedNumber).unit;
            }
